...
_MIRROR_TOKEN_SEQUENCE = count()


def _new_mirror_token() -> int:
    """Return a process-local identity for one logical state snapshot."""

    return next(_MIRROR_TOKEN_SEQUENCE)
...
class RenewableHuberState:
    ...
    _mirror_token: int = field(
        default_factory=_new_mirror_token,
        init=False,
        repr=False,
        compare=False,
    )
    ...
    @property
    def mirror_token(self) -> int:
        """Return the process-local identity of this state snapshot.

        Native backends use this scalar to decide whether their resident engine
        mirrors this exact state. It is intentionally excluded from checkpoint
        data and regenerated by :meth:`copy`.
        """

        return self._mirror_token
